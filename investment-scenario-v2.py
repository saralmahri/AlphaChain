...
correlation_threshold = 0.3  # Positive correlation threshold
...
training_window_days = 365  # Example: 1 year
buffer_days = 30  # Buffer to account for any initial missing data


# OEM tickers
oem_tickers = ['A005930', 'TWSE','AMD','INTC']  # OEM tickers in yfinance
oem_supplier_mapping = {
    'A005930':'KOSE:A005930',
    'TWSE':'TWSE:2330',
    'AMD':'NasdaqGS:AMD',
    'INTC':'NasdaqGS:INTC',
}
...
def process_supplier_data(oem_ticker, csv_path, training_start_date, investment_end_date):
    ...
    suppliers['Ticker Extracted'] = suppliers['Customer Name'].str.extract(r'\((?:[A-Z]+:)?([A-Za-z0-9]+)\)')


    # Check if the extracted ticker matches the OEM ticker
    supplier_oem_ticker = oem_supplier_mapping.get(oem_ticker, oem_ticker)

    # Include all suppliers with valid tickers
    relevant_suppliers = suppliers[suppliers['Exchange:Ticker Symbol'] != '-']
...
# Perform Analysis
def perform_analysis(period_years, training_window_days=365):
    ...
    # Define training start date based on fixed training window
    training_start = investment_start - timedelta(days=training_window_days) - timedelta(days=buffer_days)
    ...
    for oem_ticker in oem_tickers:
        csv_path = f"G:/My Drive/SarAI/Demo/Investment Scenario/{oem_ticker}_suppliers_filtered.csv"
    ...
    investment_annualization_factor = TRADING_DAYS_PER_YEAR  # Typically 252
    ...
    sharpe_ratio_supply_chain = (
        (portfolio_sharpe_supply_chain.mean() * TRADING_DAYS_PER_YEAR - risk_free_rate) /
        (portfolio_sharpe_supply_chain.std() * np.sqrt(TRADING_DAYS_PER_YEAR))
    )
    sharpe_ratio_non_supply_chain = (
        (portfolio_sharpe_non_supply_chain.mean() * TRADING_DAYS_PER_YEAR - risk_free_rate) /
        (portfolio_sharpe_non_supply_chain.std() * np.sqrt(TRADING_DAYS_PER_YEAR))
    )

    # Calculate Volatility (Standard Deviation of Returns)
    volatility_supply_chain = portfolio_sharpe_supply_chain.std() * np.sqrt(TRADING_DAYS_PER_YEAR)
    volatility_non_supply_chain = portfolio_sharpe_non_supply_chain.std() * np.sqrt(TRADING_DAYS_PER_YEAR)
    ...
    plt.show()
        ...
        # Define bar colors
    bar_colors = ['#ff7f0e', '#1f77b4']  # Consistent colors for Non-Supply Chain and Supply Chain-Enriched portfolios

    # ====================== Plot Sharpe Ratio ======================
    plt.figure(figsize=(10, 6))
    plt.bar(['Non Supply Chain', 'Supply Chain-Enriched'],
            [sharpe_ratio_non_supply_chain, sharpe_ratio_supply_chain],
            color=bar_colors, alpha=0.9)
    plt.title("Portfolio Sharpe Ratio Comparison", fontsize=16, fontweight='bold')
    ...
    plt.figure(figsize=(10, 6))
    plt.bar(['Non Supply Chain', 'Supply Chain-Enriched'],
            [volatility_non_supply_chain * 100, volatility_supply_chain * 100],
            color=bar_colors, alpha=0.9)
    plt.title("Portfolio Volatility Comparison", fontsize=16, fontweight='bold')
    plt.xlabel("Portfolio Type", fontsize=14)
